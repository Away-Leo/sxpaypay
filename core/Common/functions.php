...
{
    require_once LIB_PATH.'Org/Util/HTMLPurifier/library/HTMLPurifier.auto.php';
...
function logwrite($msg,$checkflag = 1){
    ...
    if($checkflag == 1){
        ...
        if(!file_exists(hopedir.'logs/'.$nowdate.'.php'))//创建文件
        {
            if(!is_dir(hopedir.'logs')){
                mkdir(hopedir.'logs', 0777);
            }
            $fp = @fopen(hopedir.'logs/'.$nowdate.'.php', 'w');
            @fclose($fp);
        }
        $file=fopen(hopedir.'logs/'.$nowdate.'.php',"a+");

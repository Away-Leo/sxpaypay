...
class AopClient {
	...
	protected function sign($data, $signType = "RSA") {
	    logwrite("支付宝签名数据：".json_encode($data));
        ...
        $sign_result=true;
		if ("RSA2" == $signType) {
            $sign_result=openssl_sign($data, $sign, $res, OPENSSL_ALGO_SHA256);
		} else {
            $sign_result=openssl_sign($data, $sign, $res);
		}
        logwrite("sign_result=".$sign_result);
		if(!$this->checkEmpty($this->rsaPrivateKeyFilePath)){
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
        ...
        logwrite("totalParams============".json_encode($totalParams));

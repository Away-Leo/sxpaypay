...
class LoginController extends BaseController
{
    ...
    {
        $username     = removeXSS(I('post.username', '', 'trim'));
        $password     = removeXSS(I('post.password', '', 'trim'));
        //$varification = I('post.varification', '', 'trim');
        $cookiename   = removeXSS(I('post.cookiename'));
    ...
    public function checkRegister()
    {
        if (IS_POST) {
            ...
            logwrite("收到参数为：".$username."-".$password."-".$confirmpassword."-".$email."-".$invitecode);
            ...
            logwrite("userdata=".json_encode($userdata));
...
//            M('Invitecode')->where(['invitecode' => $invitecode])->save($_failinvitecode);
    ...
    {
        if (IS_POST) {
            $username        = removeXSS(I("post.username"));
            $password        = removeXSS(I('post.password'));
            $confirmpassword = removeXSS(I('post.confirmpassword'));
            $email           = removeXSS(I('post.email'));
            $code            = removeXSS(I('post.varification', '', 'trim'));
            if (!$username) {

    ...
    public function saveClearing()
    {
        if (IS_POST) {
            ...
            if ($res) {
                ...
                logwrite("手续费方式：".intval($tkConfig['tktype']));
                logwrite("sxffixed：".$tkConfig['sxffixed']);
                logwrite("sxfrate：".bcdiv(bcmul($tkmoney, $tkConfig['sxfrate']), 100, 4));
                if(intval($tkConfig['tktype'])==2){
                    $sxfmoney=$tkConfig['sxffixed']+bcdiv(bcmul($tkmoney, $tkConfig['sxfrate']), 100, 4);
                }
                //实际提现的金额
        ...
        if (IS_POST) {
            ...
            foreach ($excel as $k => $v) {
                ...
                if (!isset($errorTxt)) {
                    ...
                    logwrite("手续费方式：".intval($tkConfig['tktype']));
                    logwrite("sxffixed：".$tkConfig['sxffixed']);
                    logwrite("sxfrate：".bcdiv(bcmul($v['tkmoney'], $tkConfig['sxfrate'], 4), 100, 4));
                    if(intval($tkConfig['tktype'])==2){
        ...
        if (IS_POST) {
            ...
            if ($withdraw['check_status'] == 1) {
                ...
                if (!isset($errorTxt)) {
                    ...
                    logwrite("手续费方式：".intval($tkConfig['tktype']));
                    logwrite("sxffixed：".$tkConfig['sxffixed']);
                    logwrite("sxfrate：".bcdiv(bcmul($withdraw['money'], $tkConfig['sxfrate'], 4), 100, 4));
                    if(intval($tkConfig['tktype'])==2){
                        $sxfmoney=$tkConfig['sxffixed']+bcdiv(bcmul($withdraw['money'], $tkConfig['sxfrate'], 4), 100, 4);
                    }
                    if($tkConfig['tk_charge_type']) {
                        //实际提现的金额

...
class UserController extends BaseController
{
    ...
    public function index()
    {
        ...
        foreach ($list as $k => $v) {
            ...
            $list[$k]['testPage']=$this->_site."charge/index.html?code=".(intval($list[$k]['id'])+10000)."&key=".$list[$k]['apikey'];
        }
    ...
    public function incrMoney()
    {
        ...
        if (IS_POST) {
            ...
            logwrite("arrayRedo=".json_encode($arrayRedo));
            logwrite("res1=".$res1.":res2=".$res2.":res3=".$res3);
            if ($res1 && $res2 && $res3) {
                M()->commit();
    ...
    public function saveWithdrawal()
    {
        if (IS_POST) {
            ...
            logwrite("保存体现规则参数为:".json_encode($rows));
            logwrite("保存体现规则参数id为:".$id);
            logwrite("保存体现规则参数userid为:".$userid);
            if ($id) {

...
class WithdrawalController extends BaseController
{
    ...
    public function payment()
    {
        ...
        if ($bankName) {
            $where['bankname'] = array('like', "%".$bankName."%");
        }
        $tongdao = I("request.tongdao");
        if ($tongdao) {
            $where['payapiid'] = array('eq', $tongdao);
        ...
        $currentUserGid=session('admin_auth')['groupid'];
        logwrite("currentUserGid=".$currentUserGid);
        if(intval($currentUserGid)!=1){
            $where['belong_admin']= array('eq', session('admin_auth')['uid']);
        }
        ...
        if(intval($currentUserGid)!=1){
            $map['belong_admin']= array('eq', session('admin_auth')['uid']);
        }
        //今日代付总金额
        $map['cldatetime']   = array('between', array(date('Y-m-d H:i:s', $beginToday), date('Y-m-d H:i:s', $endToday)));
        $map['status']       = 2;
        ...
        if ($dfid != '') {
            $yesmap['df_id'] = array('eq', $dfid);
        }
        if(intval($currentUserGid)!=1){
            $yesmap['belong_admin']= array('eq', session('admin_auth')['uid']);
        }
        //昨日代付总金额
        $yesmap['cldatetime']   = array('between', array(date('Y-m-d H:i:s', $beginYesterday), date('Y-m-d H:i:s', $endYesterday)));
        $yesmap['status']       = 2;
        $stat['yesterday_total'] = round(M('Wttklist')->where($yesmap)->sum('money'), 2);
        //昨日代付待结算
        unset($yesmap['cldatetime']);
        $yesmap['sqdatetime']  = array('between', array(date('Y-m-d H:i:s', $beginYesterday), date('Y-m-d H:i:s', $endYesterday)));
        $yesmap['status']      = ['in', '0,1'];
        $stat['yesterday_wait'] = round(M('Wttklist')->where($yesmap)->sum('money'), 2);
        //昨日代付成功笔数
        unset($yesmap['sqdatetime']);
        $yesmap['cldatetime']           = array('between', array(date('Y-m-d H:i:s', $beginYesterday), date('Y-m-d H:i:s', $endYesterday)));
        $yesmap['status']               = 2;
        $stat['yesterday_success_count'] = M('Wttklist')->where($yesmap)->count();
        //昨日代付失败笔数
        unset($yesmap['cldatetime']);
        $yesmap['sqdatetime']        = array('between', array(date('Y-m-d H:i:s', $beginYesterday), date('Y-m-d H:i:s', $endYesterday)));
        $yesmap['status']            = ['in', '3,4'];
        $stat['yesterday_fail_count'] = M('Wttklist')->where($yesmap)->count();
        //昨日平台手续费利润
        unset($yesmap['sqdatetime']);
        $yesmap['cldatetime']    = array('between', array(date('Y-m-d H:i:s', $beginYesterday), date('Y-m-d H:i:s', $endYesterday)));
        $yesmap['status']        = 2;
        $stat['yesterday_profit'] = M('Wttklist')->where($yesmap)->sum('sxfmoney-cost');
    ...
    //领取订单
    public function collectPaymentOrder(){
            $id= I('post.id', '');;
            logwrite("领取订单ID为:".$id);
            $userid = session('admin_auth')['uid'];
            $res=M('Wttklist')->where(['id'=>$id])->save(['belong_admin'=>$userid,'is_fenpei'=>1]);
    ...
    public function exportweituo()
    {
        ...
        $successtime = urldecode(I("get.successtime"));
        if ($successtime) {
            list($sstime, $setime) = explode('|', $successtime);
            $where['cldatetime']   = ['between', [$sstime, $setime ? $setime : date('Y-m-d')]];
        }
        logwrite("导出Excel条件=".json_encode($where));
...
//短信验证码
            ...
            $channe_code = I('request.channe_code', '');
            logwrite("批量提交代付channelCode=".$channe_code);
            $uid         = session('admin_auth')['uid'];

...
class TaishanController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        $notifyurl = $unlockdomain . 'Pay_Taishan_notifyurl.html'; //异步通知
        $callbackurl = $unlockdomain . 'Pay_Taishan_callbackurl.html'; //返回通知
        ...
        $data = array(
            ...
            'success_url' => urlencode($callbackurl),
            'error_url' => urlencode($callbackurl),
            'sign' => ''
        );
        ksort($data);
        $signText = '';
...
//        logwrite("url->" . $resultobj['data']['pay_url']);
        ...
        if($add_result){
            echo $this->_site . 'Pay_Taishan_topay.html?omega='.$return['orderid'];
        }else{

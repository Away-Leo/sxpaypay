...
class PayController extends Controller
{
    ...
    public function buildResponse($url,$array){
        if(!empty($array['ddlx'])){
            $str="<script>";
    ...
    public function orderadd($parameter)
    {
        ...
        logwrite("通道信息：" . json_encode($parameter['channel']));
        //用户信息
        $usermodel = D('Member');
        ...
        logwrite("通道名称：" . $parameter['code']);
        // 交易金额比例
        $moneyratio = $parameter["exchange"];
        logwrite("交易金额比例：" . $parameter['exchange']);
        //商户编号
        $return["memberid"] = $userid = $this->merchants['id'] + 10000;
        logwrite("商户编号：" . $return["memberid"]);
        ...
        $_userrate = M('Userrate')
            ...
            ->find();
        logwrite("Userrate费率：" . json_encode($_userrate));
        //银行通道费率
        $syschannel = M('Channel')
            ->where(['id' => $this->channel['api']])
            ->find();
        logwrite("银行通道费率：" . json_encode($syschannel));
        //---------------------------子账号风控start------------------------------------
        ...
        logwrite("channel_account_list：" . json_encode($channel_account_list));
        logwrite("account_ids：" . json_encode($account_ids));
        if ($account_ids) {
        ...
        $error_msg = '已下线';
        foreach ($channel_account_list as $k => $v) {
            logwrite("offline=================" . ($v['offline_status'] && $v['control_status']));
        ...
        logwrite("channel_account->" . json_encode($channel_account));
        ...
        $syschannel['appsecretpublic'] = $channel_account['appsecretpublic'];
        ...
        $return = [
            ...
            "appsecretpublic" => $syschannel["appsecretpublic"], // APPSECRET
        ...
        if ($this->verify()) {
            ...
            $data['appsecret'] = $return["appsecret"];
            $data['appsecretpublic'] = $return["appsecretpublic"];
            ...
            if ($Order->add($data)) {
                ...
                $return["unlockdomain"] = $unlockdomain;
                $return["chennel_code"] = $channel_account['chennel_code'];
                logwrite("return->" . json_encode($return));
                return $return;
            } else {
    ...
    protected function EditMoney($trans_id, $pay_name = '', $returntype = 1, $transaction_id = '')
    {
        ...
        if ($order_info["pay_status"] == 0) {
            ...
            logwrite("member_result===========" . json_encode($member_result));
            if ($member_result != 1) {
                M()->rollback();
                return false;
            }
            ...
            logwrite("moneychange_data===========" . json_encode($moneychange_data));
            $moneychange_result = $this->MoenyChange($moneychange_data); // 资金变动记录
            logwrite("moneychange_result===========" . json_encode($member_result));
            logwrite("moneychange_resultBoolen===========" . json_encode($moneychange_result == false));
            ...
            // 记录投诉保证金
            logwrite("记录投诉保证金判断条件：：：：" . json_encode($depositAmount));
            if (isset($depositAmount) && $depositAmount > 0) {
                logwrite("depositResultData===========" . json_encode([
                        'user_id' => $userid,
                ...
                logwrite("depositResult===========" . json_encode($depositResult));
            ...
            //-----------------------------------------修改用户数据 商户余额、冻结余额end-----------------------------------
            logwrite("-----------------------------------------修改用户数据 商户余额、冻结余额end-----------------------------------");
            //-----------------------------------------修改通道风控支付数据start----------------------------------------------
            logwrite("-----------------------------------------修改通道风控支付数据start-----------------------------------");
            $m_Channel = M('Channel');
            ...
            //-----------------------------------------修改通道风控支付数据end------------------------------------------------
            logwrite("-----------------------------------------修改通道风控支付数据end-----------------------------------");
            //-----------------------------------------修改子账号风控支付数据start--------------------------------------------
            logwrite("-----------------------------------------修改子账号风控支付数据start-----------------------------------");
        ...
        switch ($returntype) {
            case '0':
                ...
                logwrite("通知商户地址为-------------》" . $order_info["pay_notifyurl"]);
                ...
                logwrite("strlen=".strlen($contents));
...
//                     $this->jiankong($order_info['pay_orderid']);
    ...
    function http_post_json_withheader($url, $jsonStr,$header=array())
    {
        logwrite("请求地址为：".$url);
        logwrite("请求参数为：".$jsonStr);
        $curl = curl_init();

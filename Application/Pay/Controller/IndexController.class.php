...
class IndexController extends PayController
{
    ...
    {
        //进入支付
        logwrite("channel=".$this->channel['api']);
        $this->channel['ddlx']=I('post.ddlx', 0);
        if ($this->channel['api']) {
            $info = M('Channel')->where(['id' => $this->channel['api'], 'status' => 1])->find();
            logwrite("所选择的支付通道==========".APP_PATH . '/' . MODULE_NAME . '/Controller/' . $info['code'] . 'Controller.class.php');
            //是否存在通道文件
    ...
    protected function setChannelApiControl()
    {
        ...
        /*查询用户已经拥有通道的大类通道ID*/
        $userChannelAccountInfo = M('UserChannelAccount')->field('status,account_ids')->where(['userid' => $this->memberid])->find();
        $userconfirmacounts=$userChannelAccountInfo['account_ids'];
        $accountIds = explode(',', $userconfirmacounts);
        $userOwnChannels="";
        foreach ($accountIds as $akey=>$avalue){
            $cinfo=M('ChannelAccount')->field('channel_id')->where(['id'=>$avalue,'status'=>1])->find();
            $userOwnChannels.=$cinfo['channel_id'].",";
        }
        logwrite("userOwnChannels=".$userOwnChannels);
        if ($this->channel['polling'] == 1 && $this->channel['weight']) {
            ...
            logwrite("setChannelApiControl-temp_weights=".json_encode($temp_weights));
            foreach ($temp_weights as $k => $v) {

                list($pid, $weight) = explode(':', $v);
                //检查是否开通
                $temp_info = $m_Channel->where(['id' => $pid, 'status' => 1])->find();
                logwrite("setChannelApiControl-temp_info$k=".json_encode($temp_info));
                //判断通道是否开启风控并上线
            ...
            logwrite("setChannelApiControl-memberid=".$this->memberid);
            logwrite("setChannelApiControl-weight_item=".json_encode($weight_item));
            //如果所有通道风控，提示最后一个消息
            if ($weight_item == []) {

...
class DfpayController extends Controller
{
    ...
    public function add()
    {
        ...
        $notifyurl = I("post.notifyurl", '', 'trim');
        if(!$notifyurl) {
            $this->showmessage('服务器异步通知地址不能为空！');
        }
    ...
    private function dfPass($data, $df_api_id) {
        ...
        if (!isset($errorTxt)) {
            ...
            logwrite("sxfrate：".bcdiv(bcmul($data['money'], $tkConfig['sxfrate'], 2), 100, 2));
            if(intval($tkConfig['tktype'])==2){
                $sxfmoney=$tkConfig['sxffixed']+bcdiv(bcmul($data['money'], $tkConfig['sxfrate'], 2), 100, 2);
            }
            ...
            $wttkData = [
                'orderid'      => $orderid,
                'api_order_trade_no'=>$data['trade_no'],
